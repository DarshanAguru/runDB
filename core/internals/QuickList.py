...
def encode_prev_len(prev_len: int) -> bytes:
    if prev_len < 254:
        ...
        return bytes([254]) + struct.pack("<I", prev_len)
...
def encode_entry_payload(data: bytes) -> tuple[bytes, bytes]:
    val = try_parse_int(data)
    if val is not None:
        if 0 <= val <= 12:
            return bytes([0xF0 | (val + 1)]), b""
        elif -128 <= val <= 127:
            return bytes([0xFE]), struct.pack("<b", val)
        elif -32768 <= val <= 32767:
            return bytes([0xC0]), struct.pack("<h", val)
        elif -8388608 <= val <= 8388607:
            packed = struct.pack("<i", val)
            return bytes([0xF0]), packed[:3]
        elif -2147483648 <= val <= 2147483647:
            return bytes([0xD0]), struct.pack("<i", val)
        elif -9223372036854775808 <= val <= 9223372036854775807:
            return bytes([0xE0]), struct.pack("<q", val)
    ...
    if L <= 63:
        ...
        return bytes([0x40 | (L >> 8), L & 0xFF]), data
    else:
        return bytes([0x80]) + struct.pack(">I", L), data
...
class ZipList:
    ...
    def __init__(self, ptr=None):
        if ptr is not None:
            ...
            self.ptr = MallocInternal.zmalloc(13)
            self._write_header(13, 12, 0)
            ctypes.cast(self.ptr + 12, ctypes.POINTER(ctypes.c_uint8))[0] = self.END_MARKER
    ...
    def _read_header(self):
        ...
        zltail = ctypes.cast(self.ptr + 4, ctypes.POINTER(ctypes.c_uint32))[0]
        zllen = ctypes.cast(self.ptr + 8, ctypes.POINTER(ctypes.c_uint32))[0]
    ...
    def _write_header(self, zlbytes: int, zltail: int, zllen: int):
        ...
        ctypes.cast(self.ptr + 4, ctypes.POINTER(ctypes.c_uint32))[0] = zltail
        ctypes.cast(self.ptr + 8, ctypes.POINTER(ctypes.c_uint32))[0] = zllen
    ...
    def _next_entry_offset(self, offset: int) -> int:
        ...
        _, prev_len_enc_len = decode_prev_len(self.ptr, offset)
        _, payload_len = decode_entry_payload(self.ptr, offset + prev_len_enc_len)
    ...
    def _prev_entry_offset(self, offset: int) -> int:
        if offset is None or offset <= self.HEADER_SIZE:
            return None
        prev_len, _ = decode_prev_len(self.ptr, offset)
        if prev_len == 0:
            return None
    ...
    def get_entry_data(self, offset: int) -> bytes:
        ...
        _, prev_len_enc_len = decode_prev_len(self.ptr, offset)
        data, _ = decode_entry_payload(self.ptr, offset + prev_len_enc_len)
        return data
    ...
    def _rebuild(self, elements: list[bytes]):
        ...
        for data in elements:
            enc_prev = encode_prev_len(prev_len)
            enc_enc, enc_data = encode_entry_payload(data)
            block = enc_prev + enc_enc + enc_data
            entry_blocks.append(block)
            prev_len = len(block)
            
        total_entry_len = sum(len(b) for b in entry_blocks)
        new_size = self.HEADER_SIZE + total_entry_len + 1
        
        self._resize(new_size)
        
        if len(elements) == 0:
            zltail = self.HEADER_SIZE
        else:
            zltail = self.HEADER_SIZE + sum(len(b) for b in entry_blocks[:-1])
        ...
        ctypes.cast(self.ptr + curr_offset, ctypes.POINTER(ctypes.c_uint8))[0] = self.END_MARKER
    ...
    def lpush(self, data: bytes):
        self.insert_at_offset(self.HEADER_SIZE, data)
